...
# ╔═╡ 012bf28f-c621-4f73-bd5b-acb9e1b508b2
using AxisKeysExtra

# ╔═╡ 363e1842-7947-47cc-9892-f998a14b6e13
using StructArrays
...
# ╔═╡ ef2ae7c1-e4e7-4b8d-82bf-62a9a3793cf6
md"""
## Minor improvements

Support setting a single element by names/keys ([PR 110](https://github.com/mcabbott/AxisKeys.jl/pull/110)).
"""

# ╔═╡ f0e918d0-8b96-4bc1-976b-6e855aa54433
KA = KeyedArray(randn(2, 3), x=range(0, step=10, length=2), y=5 .* (-3:-1))

# ╔═╡ d18fb707-1e5f-4514-b98d-40380dcb882a
md"""
By default in `AxisKeys`, one can set single or multiple values through square brackets indexing like `KA[x=1, y=1] = 1000`, but only multiple values through round brackets indexing, like `KA(x=10, y=:) .= [1,2,3]`.

Here, we add single-value setting with round brackets:
"""

# ╔═╡ 9db040ef-2dc2-4b24-a181-903a2fd4ccd6
KA(x=0, y=-10, :) .= 1000;

# ╔═╡ 879195dd-cde2-4894-a4d2-77755b2cfe90
KA

# ╔═╡ 232c2d08-89da-42aa-9861-26a8eb8ce0d4
md"""
## `AxisKeys` – `StructArrays` interop
"""

# ╔═╡ c24e6de0-9881-4f44-917b-71088fed317d
md"""
`StructArray`-of-`KeyedArray`s _(with the same axiskeys)_ support both `StructArray` and `KeyedArray` interfaces:
"""

# ╔═╡ 6c058a5c-955f-4e22-8a7a-522686a96d4e
SKA = StructArray(a=KeyedArray(1:5; x=11:15), b=KeyedArray(10:10:50; x=11:15))

# ╔═╡ b4f93578-2d03-404b-978d-78be6e449695
named_axiskeys(SKA)

# ╔═╡ 39abb5c3-2c22-44a2-b4fb-b152a4dec9d5
SKA.a

# ╔═╡ 6f22b18c-97b9-4f9a-bf05-cbc67bd61df2
SKA.x

# ╔═╡ 3aec762a-bb7f-470c-85fd-0ab7675d5b34
SKA[2]

# ╔═╡ 3a52a202-099d-4d1f-bb15-5ac7ff1c6555
SKA[x=2]

# ╔═╡ 9918f7c9-a106-4a9a-ac5b-c65d5a33ba43
SKA(x=13)

# ╔═╡ 9f3c072c-e00e-41b2-b8ab-9f19871a6308
md"""
## Convenience `axiskeys` functions
"""

# ╔═╡ 0ccc5b39-5e92-4416-b261-1e5f01aae0ad
md"""
`with_axiskeys()` makes functions like `argmax` and `findmax` to return `axiskeys` values instead of regular array indices:
"""

# ╔═╡ ff2e76b2-77cf-494e-ba3a-3dce9bc2a82b
KA

# ╔═╡ 73643d9d-ed97-4d26-ad81-0039b3c42be3
argmax(KA)

# ╔═╡ 33ef139e-5916-40a4-994d-66c6020068c2
with_axiskeys(argmax)(KA)

# ╔═╡ 4608db5e-794e-482c-9fda-f8ab83c2ccdb
md"""
They seamlessly work together with `Skipper` if skipping some values is desired (eg missings, nans, ...):
"""

# ╔═╡ d53f7c7a-cd44-48e0-8723-fd15d1feb740
with_axiskeys(findmax)(skip(x -> x > 100, KA))
...
md"""
## `KeyedArray` conversions
"""

# ╔═╡ 6bd7be6d-d8a1-4d7e-a4e8-9b4d803b5773
md"""
The `KeyedArray` constructor can now create `KeyedArray`s from `DimensionalData` arrays, without copying their data:
"""

# ╔═╡ d0ec4b00-ade8-47bd-b02d-6d7d79b18cbe
DA = DimArray(reshape(1:8, (4, 2)), (a=10:10:40, b=[:x, :y]));

# ╔═╡ f4d1d009-1364-4960-9df3-d4fff6a74e06
KeyedArray(DA)
...
md"""
A whole range of 1-2-3-dimensional recipes for plotting `KeyedArray`s using their `axiskeys` for plot axes:
"""

# ╔═╡ 880f829c-fdfb-11ee-386e-3954ec54a785
Aimg = KeyedArray(randn(5, 3), x=range(0, step=10, length=5)u"cm", y=5 .* (-3:-1));

# ╔═╡ 939fb93b-86bf-4006-aa6b-952e3a4d0ed7
let
	fig = Figure()
	for (i, plotf) in enumerate([heatmap, image, contour, contourf])
		plotf(fig[i,1], Aimg, axis=(width=300, height=200))
	end
	resize_to_layout!(fig)
	fig
end

# ╔═╡ 4f79b227-ef56-43a3-9ae5-9b21ae2e8ba7
let
	fig = Figure()
	for (i, plotf) in enumerate([contour3d, surface])
		plotf(fig[1, i], Aimg, axis=(width=300, height=300))
	end
	resize_to_layout!(fig)
	fig
end
...
arrows(Vec2.(Aimg, Aimg))

# ╔═╡ 9e46f4f4-7e03-47a4-9966-6f1eea9c4e39
A1d = KeyedArray([1, 5, 2, 1], x=[1, 5, 10, 20]u"km")

# ╔═╡ 868e6577-4592-457b-834d-5d99464d527c
let
	fig = Figure()
	for (i, plotf) in enumerate([scatter, lines, scatterlines, stairs, stem, barplot])
		plotf(fig[i,1], A1d, axis=(width=300, height=200))
	end
	resize_to_layout!(fig)
	fig
end

# ╔═╡ 169e6678-60a4-42d6-a0a7-ccac0c10dbe0
let
	fig = Figure()
	for (i, plotf) in enumerate([rangebars, band])
		plotf(fig[i,1], Interval.(A1d, A1d .+ 2), axis=(width=300, height=200))
	end
	resize_to_layout!(fig)
	fig
end
...
manifest_format = "2.0"
project_hash = "43b30565c833887c92160b1d5576f4cdc634faf9"
...
git-tree-sha1 = "91038cd792dc5d0558d6815940c053a353299784"
